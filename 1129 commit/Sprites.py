...
class Map(pygame.sprite.Sprite):
	def __init__(self):
		pygame.sprite.Sprite.__init__(self)
		image = pygame.image.load(os.path.join('resources', 'images/map_easy.png'))
		size_w = image.get_width()
		size_h = image.get_height()
		self.image = image
		self.mask = pygame.mask.from_surface(self.image)
		self.rect = self.image.get_rect()
		self.rect.x = 0 #WIDTH // 2 - size_w // 2
		self.rect.y = 0 #HEIGHT // 2 - size_h // 2
...
class Player(pygame.sprite.Sprite):
	...
	def update(self, wall_sprites, gate_sprites):
		...
		is_collide = pygame.sprite.collide_mask(self, map) #spritecollide(self, wall_sprites, False)
